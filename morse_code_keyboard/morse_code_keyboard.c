...
struct MorseTreeNode *create_a_node(char value, char *lable, uint16_t keyboard_code)
{
    ...
    node->value = value;
    node->lable = lable;
    node->keyboard_code = keyboard_code;
    node->left = NULL;
    node->right = NULL;
...
{
    // root
    struct MorseTreeNode *root = create_a_node('\0', "root", 0x00);

    // first layer
    root->left = create_a_node('e', "Letter E", HID_KEYBOARD_E);
    root->right = create_a_node('t', "Letter T", HID_KEYBOARD_T);

    // second layer
    root->left->left = create_a_node('i', "Letter I", HID_KEYBOARD_I);
    root->left->right = create_a_node('a', "Letter A", HID_KEYBOARD_A);
    root->right->left = create_a_node('n', "Letter N", HID_KEYBOARD_N);
    root->right->right = create_a_node('m', "Letter M", HID_KEYBOARD_M);
        ...
        if (status == FuriStatusOk)
        {
            if (event.type == InputTypePress)
            {
                ...
                case InputKeyOk:
                    ...
                    if (app_context->model->current_symbol->lable == "root")
                    {
                        // noop if you are in the root
                        break;
                    }

                    furi_hal_hid_kb_press(app_context->model->current_symbol->keyboard_code);
                    furi_hal_hid_kb_release(app_context->model->current_symbol->keyboard_code);
                    app_context->model->current_symbol = app_context->model->tree_root;
                    break;
                case InputKeyBack:
                case InputKeyMAX:
